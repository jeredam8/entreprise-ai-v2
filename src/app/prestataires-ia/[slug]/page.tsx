import { TrackedLink, ProfileView } from "@/components/TrackedLink";
...
export async function generateMetadata({ params }: PageProps) {
  ...
  if (!provider) {
    return buildMetadata({
      ...
      description:
        "Cette fiche prestataire n'existe pas ou n'est plus référencée.",
...
export default async function ProviderPage({ params }: PageProps) {
  ...
  const faits: [string, string | undefined][] = [
    ...
    [
      "Budget de départ",
      provider.minBudget > 0 ? provider.minBudgetLabel : undefined,
    ],
  ];

  return (
    <>
      <ProfileView slug={provider.slug} />
      <JsonLd
        data={{
        ...
        <div className="mt-8 grid gap-10 lg:grid-cols-[1fr_0.6fr]">
          <div>
            ...
            <p className="mt-4 flex items-center gap-2 text-sm text-muted">
              ...
              {provider.legal?.headOfficeCity ?? provider.city} · modalités
              d’intervention à confirmer
            </p>
            ...
            {provider.website ? (
              <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                ...
                <Link
                  href={`/deposer-un-projet-ia?provider=${provider.slug}&name=${encodeURIComponent(provider.name)}`}
                  className="btn-secondary"
                >
                  Parler de mon projet avec ce profil
                </Link>
              </div>
            ) : null}
          </div>
          ...
          <div className="section-heading">
            ...
            <p>
              {provider.requestedByProvider
                ? "Informations issues de la demande de référencement et du site, relues"
                : "Relevé sur le site du prestataire"}
              {provider.verifiedAt
                ? ` le ${new Date(provider.verifiedAt).toLocaleDateString("fr-FR")}`
                : ""}
              . Ces éléments sont déclaratifs : ils décrivent l'offre annoncée,
              pas une évaluation.
            </p>
          </div>
                ...
                >
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    {(valeurs as string[]).map((valeur) => (
                      <span
                        key={valeur}
                        className="rounded-md bg-soft px-2.5 py-1 text-xs text-ink"
                      >
                        {valeur}
                      </span>
                    ))}
      ...
      {!provider.legal && provider.requestedByProvider ? (
        <section className="section pt-0">
          <p className="text-sm leading-6 text-muted">
            Référencement demandé par le prestataire. Cette fiche présente son
            offre commerciale, relue sur son site. Les informations légales ne
            sont pas affichées.
          </p>
        </section>
      ) : null}
        ...
        <section className="border-y border-line bg-soft">
          <div className="section">
            <div className="section-heading">
              ...
              <p>
                ...
                {provider.verifiedAt
                  ? ` le ${new Date(provider.verifiedAt).toLocaleDateString("fr-FR")}`
                  : ""}
                . L’identité de la structure a été rapprochée de son site et des
                informations disponibles au répertoire.
              </p>
            </div>
            ...
            <p className="mt-4 text-sm leading-6 text-muted">
              ...
              <Link
                href={`/referencer-un-prestataire-ia?mode=correction&provider=${provider.slug}&name=${encodeURIComponent(provider.name)}&website=${encodeURIComponent(provider.website || "")}`}
      ...
      <section className="section pt-0">
        <div className="rounded-md border border-line bg-soft p-6">
          <h2 className="text-2xl font-semibold text-ink">
            Vous représentez {provider.name} ?
          </h2>
          <p className="mt-3 max-w-3xl text-sm leading-6 text-muted">
            La fiche de base est gratuite. Vous pouvez la corriger, compléter
            vos prestations et partager son lien.
          </p>
          <Link
            href={`/referencer-un-prestataire-ia?mode=correction&provider=${provider.slug}&name=${encodeURIComponent(provider.name)}&website=${encodeURIComponent(provider.website || "")}`}
      ...
      {similaires.length ? (
        <section className="section">
          <div className="section-heading">
            ...
            <p>
              Profils proches par le type d'acteur, la localisation ou les
              spécialités.
            </p>
          </div>
